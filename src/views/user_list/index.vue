<template>
  <div class="user_list_container">
        ...
        <el-table :data="userList" stripe style="width: 100%" border>
          ...
          <el-table-column prop="mg_state" label="状态">
            ...
            <template slot-scope="scope">
              <el-switch
                @change="
                  switchFn(scope.row.name, scope.row.id, scope.row.mg_state)
                "
    ...
    <el-dialog title="添加用户对话框" :visible.sync="addFormVisible">
      <el-form
        label-width="80px"
        :model="newUserListInfo"
        :rules="rules"
        ref="newUserListInfo"
      >
        <el-form-item label="用户名" prop="username">
          <el-input v-model="newUserListInfo.username"></el-input>
        </el-form-item>
        <el-form-item label="密码" prop="password">
          <el-input v-model="newUserListInfo.password"></el-input>
        </el-form-item>
        <el-form-item label="邮箱" prop="email">
          <el-input v-model="newUserListInfo.email"></el-input>
        </el-form-item>
        <el-form-item label="手机号" prop="mobile">
          <el-input v-model="newUserListInfo.mobile"></el-input>
        </el-form-item>
      </el-form>
      <!-- //---弹出框底部按钮 -->
      <div slot="footer" class="dialog-footer">
        <el-button @click="addFormVisible = false">取 消</el-button>
        <el-button type="primary" @click="addNewUser('newUserListInfo')"
          >确 定</el-button
        >
      </div>
    </el-dialog>
    ...
    <el-dialog title="编辑用户" :visible.sync="editFormVisible">
      ...
      >
        <el-form-item label="用户名称" prop="username">
          <el-input v-model="newUserListInfo.username" disabled></el-input>
        </el-form-item>
        <el-form-item label="邮箱" prop="email">
          <el-input v-model="newUserListInfo.email"></el-input>
        </el-form-item>
...
<script>
import { getUsersList, setSwitchType, addNewUserApi, delUserInfo, editUserInfo, setUserInfo, getrolesList } from '@/api/user_list.js'
export default {
  created () {
    // --- 页面创建就获取用户列表 ---
    this.getUsersList()
  ...
  methods: {
    ...
    async getEditUserInfo (editInfo) {
      ...
      this.newUserListInfo.username = editInfo.username
      this.newUserListInfo.email = editInfo.email
      this.newUserListInfo.mobile = editInfo.mobile
    },
    ...
    addNewUser () {
      // 表单校验
      this.$refs.newUserListInfo.validate((valid) => {
        if (valid) { // 如果成功
          this.addNewUserApi()
        } else {
          this.$message.error({
    ...
    async addNewUserApi () {
      try {
        // 发送请求
        const res = await addNewUserApi(this.newUserListInfo)
        // 刷新列表
        this.getUsersList()
        // 提示
        this.$message({
    ...
    async getUsersList () {
      try {
        // 发送ajax
        const res = await getUsersList(this.page)
        // console.log('用户列表', res)
        this.userList = await res.data.data.users// 列表
        this.pagenum = await res.data.data.pagenum// 返回回来的当前页码
        this.totalpage = await res.data.data.total// 返回回来的总记录数
        // console.log('列表', this.userList)
